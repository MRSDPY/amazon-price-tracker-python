...
def send_mail(host_name=None, port=587, smtp_id=None, smtp_pass=None, from_email="mgidshubham@gmail.com", to_email=None,
              subject="Amazon Price Compare Result", message=None, html_contact=None):
...
def send_data(isSendMail, mailTo):
    product_name = input("Enter product name : ")
    price_range = input("Enter price range (e.x: <20000 and >20000 you can enter any number) : ")
    ...
    for i in range(len(dict_data)):
        if dict_data[str(i)]["Price"] != "No Any Price Can Define On That Page.":
            if price_con == "<":
                try:
                    if int((dict_data[str(i)]["Price"].split(".")[0].replace(",", "")).replace("₹", "")) < int(
                            price_range[1:]):
    ...
    if isSendMail:
        send_mail(host_name="smtp-relay.sendinblue.com", smtp_id=os.environ.get("Email"),
                  smtp_pass=os.environ.get("Email_pass")
                  , to_email=mailTo, html_contact=make_string)
    else:
...
try:
    if sys.argv[1] == "-m":
        ...
        mailTo = sys.argv[2:]
        print(ismail)
        send_data(ismail, mailTo)
    else:
        print("Enter this followed syntax '-m example@domain.com' ")
